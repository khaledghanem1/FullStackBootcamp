...
jQuery(function($) {
  ...
  //Remember to reference the HTML for data-* attributes that you can use to keep your code DRY

  $(".reset").on("click", function(){
    var targetVar = $(this).attr("data-target");
    var element = $(this).attr("data-target-id");

    if(targetVar === "wins"){
      wins = 0;
    }
    else if(targetVar === "losses"){
      losses = 0;
    }
    else if(targetVar === "headsCount"){
      headsCount = 0;
    }
    else if(targetVar === "tailsCount"){
      tailsCount = 0;
    }

    window.localStorage.setItem(targetVar,0);
    $("#"+element).html("0");
